 ...
 *
 * 适配（2026-09-25，dsh 0.1.7-rc.2）：注入点改为多版本变体锚点。rc.2 只改了成功路径
 *   （`await resume` 后多一层 `this.liveAgent(sessionId) ??` 兜底），其余三处同形；
 *   老变体保留，live 0.1.5-rc.2 与本机 0.1.6 仍可重打。
 *
...
/**
 * 注入点。每个 edit 的 variants 里**恰好一个变体必须在文件里恰好命中一次**：
 * 0 命中 = 上游布局变了（cannotApply），2 命中 = 会只改一处留下半补丁（同样 cannotApply）。
 * 老变体一律保留，live 0.1.5-rc.2 与本机 0.1.6 仍可重打。
 *
 * 版本差异（2026-09-25 核对 0.1.7-rc.2 pristine）：只有「成功路径」变了——rc.2 在
 * `await resume` 之后多了一层 `this.liveAgent(sessionId) ??` 兜底，故多一个变体；
 * 入口闸门、类字段、终局失败三处三版本同形。
 */
const EDITS = [
  {
    what: '常量 + 失败记录表字段',
...
function pickVariant(content, edit) {
  ...
  if (matched.length !== 1 || matched[0].n !== 1) {
    ...
    cannotApply(`anchor not unique (found ${matched.length === 1 ? matched[0].n : 0}) for [${edit.what}] — variant hits: ${detail} — dsh 换版后布局变了；先人工核对再改本脚本`)
  }
  return matched[0].v
}
...
function healthy(content, file) {
  ...
  try {
    ...
    // ERR_PACKAGE_PATH_NOT_EXPORTED 同类：依赖版本被混装时（升级只换了一部分包），
    // 目标文件 import 的某个子路径在装着的旧版依赖里不存在 —— 也是依赖侧解析失败，
    // 不是本文件坏了。2026-09-25 实测：rc.2 的 dsh-api-session-controller 要
    // `dsh-session/fork`，而同胞包 dsh-session 还是 0.1.5（无该子路径导出）。
    if (/Cannot find package|ERR_MODULE_NOT_FOUND|Cannot find module|ERR_PACKAGE_PATH_NOT_EXPORTED/i.test(detail)) return 'unknown'
...
let patched = content
// 全部注入点先各自选出唯一命中的版本变体，再统一落盘，避免半补丁。
for (const edit of EDITS) {
  const chosen = pickVariant(patched, edit)
  patched = patched.replace(chosen.old, chosen.new)
}
